...
import { CalendarDays, Flame, Newspaper, Snowflake, Swords, TrendingUp } from "lucide-react";

export const metadata = {
  title: "Weekly League Hub",
  description:
    "This week in the league: matchups and scores, standings, power rankings, transactions, streaks and the latest news — on one page.",
};
  ...
  if (!hub) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <PageHeader eyebrow="The League" title="Weekly League Hub" />
        <div className="mt-8">
          <EmptyState
            icon={CalendarDays}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <PageHeader
        ...
        title="Weekly League Hub"
        description="Everything that matters this week — scores, standings, rankings, the wire and the news — on one page."
      />

      {/* ── Where the season actually is, and the week switcher ─────────── */}
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <Badge className="bg-primary text-primary-foreground">{periodLabel}</Badge>
        {!inSeason ? (
        ...
        <SectionHeading
          ...
          action={
            <Link href="/matchups" className="text-sm text-primary hover:underline">
              Every week&rsquo;s matchups →
            </Link>
          }
        >
          {hub.week != null ? `Week ${hub.week} Matchups` : "Matchups"}
        </SectionHeading>
        {hub.matchups.length === 0 ? (
          <EmptyState
            icon={Swords}
            title="No matchups scheduled yet"
            description="The schedule appears here once the season is synced."
          />
        ) : (
