...
describe("deriveFinalPlacements", () => {
  it("assigns every roster exactly one distinct place", () => {
    const result = deriveFinalPlacements(WINNERS_2023, LOSERS_2023, 6);
  ...
  it("reads the winners bracket placements as absolute", () => {
    const { byRosterId } = deriveFinalPlacements(WINNERS_2023, LOSERS_2023, 6);
  ...
  it("offsets the losers bracket past the playoff field", () => {
    const { byRosterId } = deriveFinalPlacements(WINNERS_2023, LOSERS_2023, 6);
    // Losers p=1 decides 7th/8th once six teams have been placed above.
    expect(byRosterId.get(1)).toBe(7);
    expect(byRosterId.get(9)).toBe(8);
    expect(byRosterId.get(10)).toBe(9);
    expect(byRosterId.get(8)).toBe(10);
  });

  it("records who reached the winners bracket", () => {
    const { playoffRosterIds } = deriveFinalPlacements(WINNERS_2023, LOSERS_2023, 6);
  ...
  it("returns nothing for an unplayed bracket rather than guessing", () => {
    ...
    const result = deriveFinalPlacements(unplayed, [], 6);
    expect(result.byRosterId.size).toBe(0);
    expect(result.problem).toBeUndefined();
  });

  it("refuses a bracket that would place two rosters in the same position", () => {
    // A losers bracket whose offset collides with the winners bracket: this is
    // what a wrong playoffTeamCount would look like.
    const result = deriveFinalPlacements(WINNERS_2023, LOSERS_2023, 0);
  ...
  it("ignores matchups with no placement value", () => {
    ...
    const result = deriveFinalPlacements(onlyUnplaced, [], 6);
    expect(result.byRosterId.size).toBe(0);
